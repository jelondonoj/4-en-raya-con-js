    ...
    methods: {
        ...
        async askUserGameMode() {
            ...
            const result = await Swal.fire({
                title: 'Selecciona un modo de juego',
                text: "¿Quieres jugar contra otro jugar local o contra la CPU?",
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#fdbf9c',
                cancelButtonColor: '#4A42F3',
                cancelButtonText: 'Yo Vs Otro jugador (Local)',
                confirmButtonText: 'Yo Vs La CPU'
            });
            this.canPlay = true;
        ...
        async makeMove(columnNumber) {
            ...
            if (firstEmptyRow === -1) {
                Swal.fire('No se puede poner aquí, ya no hay espacio');
                return;
            }
        ...
        getBestColumnForCpu() {
            ...
            if (winnerColumn !== -1) {
                console.log("La CPU elige la columna ganadora");
                return winnerColumn;
            }
            ...
            if (winnerColumnForAdversary !== -1) {
                console.log("La CPU elige tomar la victoria del adversario");
            ...
            if (adversaryStats.highestCount > cpuStats.highestCount) {
                console.log("La CPU elige obtener la puntuación más alta del adversario");
                ...
                console.log("La CPU elige el conteo más alto");
                return cpuStats.columnIndex;
            }
            const centralColumn = this.getCentralColumn(this.board);
            if (centralColumn !== -1) {
                console.log("La CPU elige la columna central");
                return centralColumn;
            }
            // Finally we return a random column
            console.log("La CPU elige una columna aleatoria");
        ...
        async showWinner() {
            if (this.currentPlayer === PLAYER_1) {
                await Swal.fire('El ganador es el jugador 1');
            } else {
                await Swal.fire('El ganador es el jugador 2');
            }
        },
        async showTie() {
            await Swal.fire('Tie');
